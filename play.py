            ...
            if done:
                if args.timer:
                    logger.info("Epoch {}, Agents {} have finished after {}. "
                                ...
                                "Bye Bye".format(epoch, str([i for i, x in enumerate(dones) if x])[1:-1], epoch_counter, avg_time))
                else:
                    logger.info("Epoch {}, Agents {} have finished after {}. "
                                "Now everyone most die. "
                                "Bye Bye".format(epoch, str([i for i, x in enumerate(dones) if x])[1:-1], epoch_counter))
